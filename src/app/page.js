...
export default function Home() {
  ...
  return (
    <Flex flexDirection={["column", "column", "row"]} padding={["1rem","2rem", "2rem", "3rem"]} minHeight="100vh" minWidth="100vw" backgroundColor="#eaeaea">
      {/*left box*/}
      <Box display="flex" flexDirection="column" backgroundColor="#f4f4f4" padding="1rem" borderRadius={["25"]} width={["100%"]}>
        <Input placeholder='Search a city' onChange={handleChange} marginY="1rem"/>
      ...
      <>        <Flex justifyContent="space-between" margin="0.5rem">
        ...
        <Flex justifyContent="center" marginY="1rem">
          <Heading size={['3xl', '3xl', '4xl']} color="#292929">{`${forecast?.list[0].main.temp}° C`}</Heading>
        ...
        <Flex flexDirection={["column", "column", "row"]} marginTop="2rem">
          ...
          condensedForecast?.map(e => {
            return(
           <ForeCastItem element={e} key={e.dt}/>
          )
          })
        }
